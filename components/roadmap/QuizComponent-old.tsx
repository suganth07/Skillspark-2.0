...
export function QuizComponent({ quizId, roadmapId, onQuizComplete, onBack }: QuizComponentProps) {
  ...
  const renderQuestion = (question: typeof questions.$inferSelect, index: number) => {
    ...
    return (
      <Card key={questionId} className="mb-4">
        ...
        <CardContent className="space-y-4">
          ...
          {question.type === 'multiple_choice' && questionData.options && (
            <RadioGroup 
          ...
          {question.type === 'text' && (
            <Input
              ...
              onChangeText={(text) => handleAnswerChange(questionId, text)}
          ...
          {question.type === 'code' && questionData.codeSnippet && (
      ...
      <View className="space-y-6">
        <Card className={`border-2 ${quizResult.passed ? 'border-green-200 bg-green-50' : 'border-red-200 bg-red-50'}`}>
          ...
          <CardContent className="space-y-4">
            ...
            {quizResult.passed && quizResult.completedPrerequisites !== undefined && (
              <View className="bg-green-50 p-4 rounded-lg border border-green-200">
                <Text className="font-medium text-green-800 mb-2">Progress Update</Text>
                <Text className="text-green-700">
                  ✅ Completed: {quizResult.completedPrerequisites}/{quizResult.totalPrerequisites} prerequisites
                </Text>
                {quizResult.nextUnlockedTopics && quizResult.nextUnlockedTopics.length > 0 && (
                  <Text className="text-green-700 mt-1">
                    🔓 Unlocked: {quizResult.nextUnlockedTopics.join(', ')}
                  </Text>
                )}
              </View>
            )}
